...
export default function Booking({ bookings, refreshBookings, loadingBookings }) {
  ...
  const USER_ID = currentUser?.username || 'unknown'
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      const { data: insertedBooking, error: insErr } = await supabase.from('bookings').insert({
      ...
      sendTicketEmail(currentUser, insertedBooking, null, qrData); // Fire and forget

      setCurrentStep(4) // Show success step
      setSuccessMsg('Booking submitted! Awaiting admin approval. A confirmation email has been sent.')
              ...
              <div className="checkout-details animate-fade-in">
                ...
                <div className="checkout-qr-container" style={{ marginTop: '24px' }}>
                  <div className="checkout-qr-glow">
                    <img src="/phonepe.jpeg" alt="PhonePe QR Code" style={{ width: '150px', height: '150px', borderRadius: '8px' }} />
                  </div>
            ...
            <div className="table-wrap" style={{ marginTop: '16px' }}>
              <table>
                ...
                <tbody>
                  {myBookings.map(b => (
                    <tr key={b.id}>
                      <td style={{ fontWeight: '600' }}>Seat {b.seat_number}</td>
                      <td>
                        {b.status === 'approved' ? (
                          <button className="btn-primary" style={{ padding: '6px 12px', fontSize: '12px' }} onClick={() => setViewingTicket(b)}>
                            View Ticket
                          </button>
                        ) : (
